...
const validateOrcamento = (req, res, next) => {
  const { cliente, telefone, cidade, data_evento, quantidade_convidados, descricao } = req.body;
  ...
  // Validar quantidade de convidados
  if (isNaN(quantidade_convidados) || quantidade_convidados < 1) {
    return res.status(400).json({ message: 'Quantidade de convidados deve ser um número maior que 0' });
  }
...
router.post('/', validateOrcamento, async (req, res) => {
  try {
    ...
    const result = await executeQuery(
      ...
      [
        cliente, telefone, cidade, data_evento, quantidade_convidados,
        descricao, valor || 0, valor_base || 0, taxa_servico || 0, incluir_taxa_servico || false,
...
router.put('/:id', validateOrcamento, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      cliente,
    ...
    await executeQuery(
      ...
      [cliente, telefone, cidade, data_evento, quantidade_convidados, descricao, valor || 0, status, id, req.session.userId]
    );
